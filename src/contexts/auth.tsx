...
export const AuthProvider = ({ children }) => {
  ...
  useEffect(() => {
    async function loadUserFromlocalstorage() {
      const token = (typeof window !== 'undefined') ? localStorage.getItem('token') : "", user_data = localStorage.getItem('user_data')
      if (token && user_data) {
        setUser(user_data)
...
export const ProtectRoute = ({ children }) => {
  ...
  const router = useRouter()
  const token = (typeof window !== 'undefined') ? localStorage.getItem('token') : "", user_data = (typeof window !== 'undefined') ? localStorage.getItem('user_data') : ""
  const unProtectedRoutes = themeConfig.unProtectedRoutes
  const loginPageRoute = themeConfig.loginPageRoute
